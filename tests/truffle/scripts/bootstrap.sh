...
function generate_genesis() {
     ...
     node generate-genesis.js --chainid ${ASC_CHAIN_ID} --ascChainId ${chainIDHex}
}
...
function init_genesis_data() {
     ...
     sed -i -e "s/{{NetworkId}}/${ASC_CHAIN_ID}/g" ${workspace}/storage/${node_id}/config.toml
     if [ "${node_id}" == "asc-rpc" ]; then
...
for((i=1;i<=${NUMS_OF_VALIDATOR};i++)); do
     init_validator "asc-validator${i}"
done
...
init_genesis_data asc-rpc asc-rpc

for((i=1;i<=${NUMS_OF_VALIDATOR};i++)); do
     init_genesis_data validator "asc-validator${i}"
done
