...
truffle-test:
	docker build . -f ./docker/Dockerfile --target asc-genesis -t asc-genesis
	docker build . -f ./docker/Dockerfile --target asc -t asc
	docker build . -f ./docker/Dockerfile.truffle -t truffle-test
	docker-compose -f ./tests/truffle/docker-compose.yml up genesis
	docker-compose -f ./tests/truffle/docker-compose.yml up -d asc-rpc asc-validator1
	sleep 30
