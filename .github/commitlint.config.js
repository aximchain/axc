const validateTypeNums = (parsedCommit) => {
  ...
  if (!parsedCommit.type) {
    return [
      false,
      'invalid commit message, should be like "name: descriptions.", yours: "' +
        parsedCommit.raw +
        '"',
    ];
  }
  ...
  for (var i = 0; i < types.length; i++) {
    if (types[i].toLowerCase() == "wip" || types[i].toLowerCase() == "r4r") {
      return [
        false,
        "R4R  or WIP is not acceptable, no matter upper case or lower case",
      ];
    }
  }
  return [true, ""];
...
module.exports = {
  ...
  helpUrl: "https://github.com/aximchain/asc/tree/develop/docs/lint/commit.md",
};
